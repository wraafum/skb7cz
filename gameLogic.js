...
export function startDialogue(dialogueId) {
    ...
    // ZMIANA: Pokaż sekcję aktywnej interakcji zamiast current-dialogue-area
    if (dom.activeInteractionSectionEl) {
        dom.activeInteractionSectionEl.classList.remove('hidden');
...
export function selectOption(dialogueId, optionId) {
    ...
    timeoutManager.set('dialogueEnd', () => {
        // ZMIANA: Ukryj sekcję aktywnej interakcji zamiast current-dialogue-area
        if (dom.activeInteractionSectionEl) {
            dom.activeInteractionSectionEl.classList.add('hidden');
        }
        clearActiveDialogue();
        ui.updateDisplay();
    }, 6000);
...
// POPRAWKA: Wdrożenie nowej, poprawnej logiki odblokowywania
export function applyStageSpecificUnlocks() {
    // Check ALL upgrades regardless of initialInteractionCompleted
    gameDefinitions.upgrades.forEach(upgDef => {
        const upgState = gameState.upgradesState.find(s => s.id === upgDef.id);
        // Check only those that are not yet unlocked and purchased
        if (upgState && !upgState.unlocked && !upgState.purchased) {
            let shouldUnlock = false;
            // Unlock based on stage
            if (upgDef.requiredStage !== undefined && gameState.lilithStage >= upgDef.requiredStage) {
                shouldUnlock = true;
            }
            // Unlock based on corruption
            if (upgDef.corruptionRequired !== undefined && gameState.corruption >= upgDef.corruptionRequired) {
                shouldUnlock = true;
            }
            // Always unlock if marked as initially unlocked
            if (upgDef.initialUnlockedState) {
                shouldUnlock = true;
            }

            if (shouldUnlock) {
                setUpgradeUnlocked(upgDef.id, true);
            }
        }
    });

    // Same for research projects
    gameDefinitions.researchProjects.forEach(rpDef => {
        ...
        if (rpState && !rpState.unlocked && !rpState.isCompleted) {
            let shouldUnlock = false;
            if (rpDef.requiredStage !== undefined && gameState.lilithStage >= rpDef.requiredStage) {
                shouldUnlock = true;
            }
            if (rpDef.initialUnlockedState) {
                shouldUnlock = true;
            }
            if (shouldUnlock) {
                setResearchProjectUnlocked(rpDef.id, true);
            }
        }
    });
...
export function buyUpgrade(upgradeId) {
    ...
    if (gameState.essence < upgradeDef.cost || (upgradeDef.darkEssenceCost && gameState.darkEssence < upgradeDef.darkEssenceCost)) {
        return;
    }
    ...
    if (upgradeDef.type === 'choice_unlock') {
        ...
        if (upgradeDef.type === 'elite_minion_recruitment') {
             setUpgradePurchased(upgradeId, true);
        }
    } else {
        ...
        if (upgradeDef.onPurchase) {
            upgradeDef.onPurchase(gameState, gameDefinitions);
        }
        if (upgradeDef.type !== 'minion_training' && upgradeDef.type !== 'elite_minion_recruitment') {
            setUpgradePurchased(upgradeId, true);
        }
    }
...
function completeResearch(projectId) {
    ...
    if (projectDef.unlocksInteractionId) {
        ui.markDialoguesDirty();
    }
    applyStageSpecificUnlocks();
}
...
export function selectUpgradeChoice(groupId, choiceId) {
    ...
    if (unlockingUpgradeDef) {
        setUpgradePurchased(unlockingUpgradeDef.id, true);
    }
...
function resolveTemptation(temptationId, isApprenticeMission = false) {
    ...
    if (!isApprenticeMission) {
        ...
        if (gameState.playerChoiceFlags.includes('rp_suggestion_basics_unlocked')) {
            successChance += BALANCE_MODIFIERS.temptation.suggestionBasicsBonus;
        }
        if (gameState.playerChoiceFlags.includes('advanced_seduction_unlocked')) {
            successChance += BALANCE_MODIFIERS.temptation.advancedSeductionBonus;
        }
    } else {
